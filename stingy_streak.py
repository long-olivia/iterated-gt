...
results=[]
const_history=[
    {"role": "user", "content": "It is currently the game's 19th round. GPT-4o chose to contribute 0 points for the past three rounds, even after you contributed 5 points two rounds ago to maintain cooperative behavior while preserving your self-interests, but it has been consistently contributing 10 points for several rounds earlier on. You now have ten points. How many of these points do you choose to contribute?"}
]
#main

def run(prompt):
    global total, results
    total = 0
    rounds_data=[]
    print(f"\n--- Starting Game with {MODEL_NAME}: {prompt.capitalize()} ---")
    sys_prompt=initialize(prompt)
    model_history=[{"role": "system", "content": sys_prompt}]
    model_history+=const_history
    for i in range(NUMBER_ROUNDS):
        print(f"\n--- Round {i + 1}/{NUMBER_ROUNDS} ---")
        # user_content = (
        #     # f"It is Round {i + 1}. You currently have {total} total points accumulated. "
        #     # + (f"In the last round, you contributed {last_contribution}. "
        ...
        model_response = call(MODEL_NAME, model_history)
        contribution = model_response["contribution"]
        reasoning = model_response["reasoning"]
        print(f"Model {MODEL_NAME} contributed: {contribution} (Reasoning: {reasoning})")


        # model_gain = payoff(contribution, #constant)
        # total+=model_gain

        print(f"Round {i + 1} Results:") #add constant depending on how many fabricated rounds
        # print(f"  Total contribution this round: {contribution + #constant}")
        # print(f"  Model {MODEL_NAME} gained: {model_gain} points (Total: {total})")

        # last_contribution = contribution
        # last_gain = model_gain
        # last_total_contribution = a_contribution + #constant

        # model_history.append({"role": "assistant", "content": json.dumps(model_response)})

        # outcome = (
        #     f"In Round {i + 1}, you contributed {contribution}. "
        #     f"The total contribution was {last_total_contribution}. "
        #     f"You gained {model_gain} points this round. "
        #     f"Your new total accumulated points are {total}. "
        # )

        # model_history.append({"role": "user", "content": outcome})
        
        # rounds_data.append({
        #     "round": i + 1, #plus some constant
    ...
    print(f"\n--- Game End ---")
    print(f"Final Total points: {total}")
...
if __name__ == "__main__":
    if not OPENROUTER_API_KEY:
        ...
        if len(sys.argv) < 2:
            print("Usage: python stingy_streak.py <prompt>")
        else:
            prompt=sys.argv[1]
            run(prompt)
            timestamp=datetime.now().strftime("%Y%m%d_%H%M%S")
            output_filename = f"stingy_results/{prompt}_4o_{timestamp}.json"
            os.makedirs("stingy_results", exist_ok=True)
            try:
